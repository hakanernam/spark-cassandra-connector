...
class CassandraRDD[R] private[connector] (
  ...
  private def copy(columnNames: ColumnSelector = columnNames,
    ...
    new CassandraRDD(sc, connector, keyspaceName, tableName, columnNames, where, limit, readConf)
  }
  ...
  def as[B : ClassTag, A0 : TypeConverter](f: A0 => B): CassandraRDD[B] = {
    ...
    new CassandraRDD[B](sc, connector, keyspaceName, tableName, columnNames, where, limit, readConf)
  }
